...
#import "conversions/PasteTextInputConversions.h"
...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps
{
  ...
  if (newTextInputProps.keyboardAppearance != oldTextInputProps.keyboardAppearance) {
    _backedTextInputView.keyboardAppearance =
        keyboardAppearanceFromString(newTextInputProps.keyboardAppearance);
  }
  ...
  if (newTextInputProps.clearButtonMode != oldTextInputProps.clearButtonMode) {
    _backedTextInputView.clearButtonMode =
        textFieldViewModeFromString(newTextInputProps.clearButtonMode);
  }
           ...
           oldLayoutMetrics:(const LayoutMetrics &)oldLayoutMetrics
{
  ...
  if (_eventEmitter) {
      ...
      eventEmitter->onContentSizeChange([self _contentSizeChangeMetrics]);
  }
}
...
- (void)textInputDidEndEditing
{
  if (_eventEmitter) {
      const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onEndEditing([self _textEventMetrics]);
...
{
  const PasteTextInputSubmitBehavior submitBehavior = [self getSubmitBehavior];
  const BOOL shouldSubmit = submitBehavior == PasteTextInputSubmitBehavior::Submit || submitBehavior == PasteTextInputSubmitBehavior::BlurAndSubmit;
...
{
  return [self getSubmitBehavior] == PasteTextInputSubmitBehavior::BlurAndSubmit;
}
...
- (void)textInputDidChange
{
  ...
  if (_eventEmitter) {
    const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onChange([self _changeEventMetrics]);
  }
}
...
- (void)textInputDidChangeSelection
{
  ...
  if (_eventEmitter) {
    const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onSelectionChange([self _selectionChangeMetrics]);
  }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  if (_eventEmitter) {
    const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onScroll([self _scrollMetrics]);
  }
}
...
#pragma mark - Event Metrics Helpers
...
- (PasteTextInputEventEmitter::OnEndEditing)_textEventMetrics
{
    return {
        .target = (int)self.tag,
        .text = RCTStringFromNSString(_backedTextInputView.attributedText.string),
...
- (PasteTextInputEventEmitter::OnSelectionChange)_selectionChangeMetrics
{
    auto range = [self _selectionRange];
    return {
        .target = (int)self.tag,
        .selection = {
...
- (PasteTextInputEventEmitter::OnScroll)_scrollMetrics
{
...
- (PasteTextInputEventEmitter::OnContentSizeChange)_contentSizeChangeMetrics
{
    CGSize contentSize = _backedTextInputView.contentSize;
    return {
        .target = (int)self.tag,
...
- (PasteTextInputSubmitBehavior)getSubmitBehavior
{
  const auto &props = static_cast<const PasteTextInputProps &>(*_props);
