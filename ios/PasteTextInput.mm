...
#import <react/renderer/components/PasteTextInputSpecs/ShadowNodes.h>
...
#pragma mark - Event Handler Setters (Legacy Interop Mode)
// In legacy interop mode, the ViewManager exports event handlers and tries to set them
// These are no-op setters since events are handled by the Fabric event emitter

- (void)setOnChange:(RCTBubblingEventBlock)onChange {
  // No-op: Events handled by Fabric
}

- (void)setOnPaste:(RCTBubblingEventBlock)onPaste {
  // No-op: Events handled by Fabric
}

- (void)setOnSelectionChange:(RCTDirectEventBlock)onSelectionChange {
  // No-op: Events handled by Fabric
}
...
- (void)updateProps:(const Props::Shared &)props oldProps:(const Props::Shared &)oldProps
{
  ...
  if (newTextInputProps.autoCapitalize != oldTextInputProps.autoCapitalize) {
    _backedTextInputView.autocapitalizationType =
        RCTUITextAutocapitalizationTypeFromAutocapitalizationType(convertAutoCapitalize(newTextInputProps.autoCapitalize));
  }

  if (newTextInputProps.autoCorrect != oldTextInputProps.autoCorrect) {
    _backedTextInputView.autocorrectionType =
        RCTUITextAutocorrectionTypeFromOptionalBool(newTextInputProps.autoCorrect);
  }

  if (newTextInputProps.contextMenuHidden != oldTextInputProps.contextMenuHidden) {
    _backedTextInputView.contextMenuHidden = newTextInputProps.contextMenuHidden;
  }

  if (newTextInputProps.editable != oldTextInputProps.editable) {
    _backedTextInputView.editable = newTextInputProps.editable;
  }

  if (newTextInputProps.enablesReturnKeyAutomatically !=
      oldTextInputProps.enablesReturnKeyAutomatically) {
    _backedTextInputView.enablesReturnKeyAutomatically = newTextInputProps.enablesReturnKeyAutomatically;
  }

  if (newTextInputProps.keyboardAppearance != oldTextInputProps.keyboardAppearance) {
    _backedTextInputView.keyboardAppearance =
        RCTUIKeyboardAppearanceFromKeyboardAppearance(
            newTextInputProps.keyboardAppearance == "dark" ? KeyboardAppearance::Dark : KeyboardAppearance::Light);
  }

  if (newTextInputProps.spellCheck != oldTextInputProps.spellCheck) {
    _backedTextInputView.spellCheckingType =
        RCTUITextSpellCheckingTypeFromOptionalBool(newTextInputProps.spellCheck);
  }

  if (newTextInputProps.caretHidden != oldTextInputProps.caretHidden) {
    _backedTextInputView.caretHidden = newTextInputProps.caretHidden;
  }

  if (newTextInputProps.clearButtonMode != oldTextInputProps.clearButtonMode) {
    _backedTextInputView.clearButtonMode =
        RCTUITextFieldViewModeFromTextInputAccessoryVisibilityMode(
            newTextInputProps.clearButtonMode == "while-editing" ? TextInputAccessoryVisibilityMode::WhileEditing : TextInputAccessoryVisibilityMode::Never);
  }

  if (newTextInputProps.scrollEnabled != oldTextInputProps.scrollEnabled) {
    _backedTextInputView.scrollEnabled = newTextInputProps.scrollEnabled;
  }

  if (newTextInputProps.secureTextEntry != oldTextInputProps.secureTextEntry) {
    _backedTextInputView.secureTextEntry = newTextInputProps.secureTextEntry;
  }

  if (newTextInputProps.keyboardType != oldTextInputProps.keyboardType) {
    _backedTextInputView.keyboardType = RCTUIKeyboardTypeFromKeyboardType(convertKeyboardType(newTextInputProps.keyboardType));
  }

  if (newTextInputProps.returnKeyType != oldTextInputProps.returnKeyType) {
    _backedTextInputView.returnKeyType = RCTUIReturnKeyTypeFromReturnKeyType(convertReturnKeyType(newTextInputProps.returnKeyType));
  }

  if (newTextInputProps.textContentType != oldTextInputProps.textContentType) {
    _backedTextInputView.textContentType = RCTUITextContentTypeFromString(newTextInputProps.textContentType);
  }

  if (newTextInputProps.passwordRules != oldTextInputProps.passwordRules) {
    _backedTextInputView.passwordRules = RCTUITextInputPasswordRulesFromString(newTextInputProps.passwordRules);
  }

  if (newTextInputProps.smartInsertDelete != oldTextInputProps.smartInsertDelete) {
    _backedTextInputView.smartInsertDeleteType =
        RCTUITextSmartInsertDeleteTypeFromOptionalBool(newTextInputProps.smartInsertDelete);
  }
...
- (void)updateState:(const State::Shared &)state oldState:(const State::Shared &)oldState
{
  ...
  // Note: PasteTextInputState is an empty struct in RN 0.81+
  // State management has been simplified
}
           ...
           oldLayoutMetrics:(const LayoutMetrics &)oldLayoutMetrics
{
  ...
  if (_eventEmitter) {
      ...
      eventEmitter->onContentSizeChange([self _contentSizeChangeEventMetrics]);
  }
}
...
- (void)textInputDidBeginEditing
{
  ...
  if (_eventEmitter) {
      const auto eventEmitter = [self getEventEmitter];
      eventEmitter->onFocus([self _focusEventMetrics]);
  }
}
...
- (void)textInputDidEndEditing
{
  if (_eventEmitter) {
      const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onEndEditing([self _endEditingEventMetrics]);
    eventEmitter->onBlur([self _blurEventMetrics]);
  }
}
...
- (BOOL)textInputShouldSubmitOnReturn
{
  ...
  if (_eventEmitter && shouldSubmit) {
    const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onSubmitEdition([self _submitEventMetrics]);
  }
  return shouldSubmit;
}
...
- (NSString *)textInputShouldChangeText:(NSString *)text inRange:(NSRange)range
{
  ...
  if (!_backedTextInputView.textWasPasted) {
    if (_eventEmitter) {
      const auto &textInputEventEmitter = static_cast<const PasteTextInputEventEmitter &>(*_eventEmitter);
      textInputEventEmitter.onKeyPress({
          .target = Tag{},
          .key = RCTStringFromNSString(text),
      });
    }
  }
...
- (void)textInputDidChange
{
  ...
  if (_eventEmitter) {
    const auto &textInputEventEmitter = static_cast<const PasteTextInputEventEmitter &>(*_eventEmitter);
    textInputEventEmitter.onChange([self _changeEventMetrics]);
  }
}
...
- (void)textInputDidChangeSelection
{
  ...
  if (_eventEmitter) {
    const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onSelectionChange([self _selectionChangeEventMetrics]);
  }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  if (_eventEmitter) {
    const auto eventEmitter = [self getEventEmitter];
    eventEmitter->onScroll([self _scrollEventMetrics]);
  }
}
...
{
    RCTPasteTextInputHandleCommand(self, commandName, args);
}
...
// Event metric helpers for RN 0.81+
- (PasteTextInputEventEmitter::OnFocus)_focusEventMetrics
{
  return {.target = Tag{}};
}

- (PasteTextInputEventEmitter::OnBlur)_blurEventMetrics
{
  return {.target = Tag{}};
}

- (PasteTextInputEventEmitter::OnEndEditing)_endEditingEventMetrics
{
  return {.target = Tag{}, .text = RCTStringFromNSString(_backedTextInputView.attributedText.string)};
}

- (PasteTextInputEventEmitter::OnSubmitEdition)_submitEventMetrics
{
  return {.target = Tag{}, .text = RCTStringFromNSString(_backedTextInputView.attributedText.string)};
}

- (PasteTextInputEventEmitter::OnChange)_changeEventMetrics
{
  return {
      .target = Tag{},
...
- (PasteTextInputEventEmitter::OnSelectionChange)_selectionChangeEventMetrics
{
  ...
  return {
      .target = Tag{},
      .selection = {.start = static_cast<int>(start), .end = static_cast<int>(end)}};
}

- (PasteTextInputEventEmitter::OnScroll)_scrollEventMetrics
{
  ...
  CGSize layoutSize = _backedTextInputView.bounds.size;
  ...
  return {
      ...
      .layoutMeasurement = {.width = layoutSize.width, .height = layoutSize.height},
      .responderIgnoreScroll = false,
      .velocity = {.x = 0, .y = 0}};
}

- (PasteTextInputEventEmitter::OnContentSizeChange)_contentSizeChangeEventMetrics
{
  CGSize contentSize = _backedTextInputView.contentSize;
  return {
      .target = Tag{}, .contentSize = {.width = static_cast<int>(contentSize.width), .height = static_cast<int>(contentSize.height)}};
}

- (void)_updateState
{
  // Note: PasteTextInputState is an empty struct in RN 0.81+
  // Increment event count for change tracking
...
- (void)_restoreTextSelection
{
  ...
  // In RN 0.81+, selection is not optional, check if it's set (non-zero)
...
-(void)_setOnPaste{
    ...
    _backedTextInputView.onPaste = ^(NSDictionary *body) {
        ...
        const auto eventEmitter = [strongSelf getEventEmitter];
        if (eventEmitter) {
...
{
  const auto &props = static_cast<const PasteTextInputProps &>(*_props);
  const SubmitBehavior submitBehaviorDefaultable = convertSubmitBehavior(props.submitBehavior);
